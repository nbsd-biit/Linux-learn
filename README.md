...
·异步通讯
·.msg文件定义通信的信息    
编程过程：1.编程接口初始化 2.创建节点并初始化 3.创建发布者对象 4.创建并填充话题信息 5.发布话题消息 6.销毁节点并关闭接口   
1.编程接口初始化 2.创建节点并初始化 3.创建订阅者对象 4.回调函数处理话题数据 5.销毁节点并关闭接口     
rqt_graph #可视化UI，可以查看话题和节点之间的通讯   
### 服务
·客户端服务器（CS模型）   
·同步通讯机制   
·服务器唯一，客户端不一定   
编程过程：1.编程接口初始化 2.创建节点并初始化 3.创建客户端对象 4.创建并发送请求数据 5.等待服务器应答并发送数据 6.销毁节点并关闭连接   
1.编程接口初始化 2.创建节点并初始化 3.创建服务器端对象 4.回调函数处理话题数据 5.向客户端反应并应答结果 6.销毁节点并关闭接口     
### 通讯接口
通信接口定义：话题.msg，服务.srv，动作.action
